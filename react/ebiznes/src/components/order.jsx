
import { Form, Button, Row, Col } from 'react-bootstrap';
import { withRouter, useLocation } from 'react-router-dom'
import axios from "axios";
import React, { useState, useEffect } from "react";

export function Order() {
    const location = useLocation();
    const [order, setOrder] = useState({});

    let orderId = 0
    useEffect(() => {
        orderId = location.state.order_id
    }, [location]);

    const getOrder = async () => {
        try {
            const response = await axios.get(`http://localhost:9000/order/` + orderId).then(res => {
                setOrder(res.data)
            });
        } catch (e) {
            console.log(e);
        }
    }

    useEffect(() => {
        getOrder();
    }, {})
    
    function getProducts(){
        if (order.products === undefined || order.products === null)
            return []
        return order.products
    }

    return (
        <>
            <link
                rel="stylesheet"
                href="https://maxcdn.bootstrapcdn.com/bootstrap/4.5.0/css/bootstrap.min.css"
                integrity="sha384-9aIt2nRpC12Uk9gS9baDl411NQApFmC26EwAOH8WgZl5MYYxFfc+NcPb1dKGj7Sk"
                crossorigin="anonymous"
            />
            <div className="px-md-5 pt-md-4">
                <h1>Zamówienie: {order.id}</h1>
                {
                    getProducts().map((product) => {
                        return (
                            <Row className="my-1">
                                <Col><b>Nazwa:</b> {product.name}</Col>
                                <Col><b>Opis:</b> {product.description}</Col>
                                <Col><b>Cena:</b> {Math.round((product.price + Number.EPSILON) * 100) / 100}zł</Col>
                            </Row>
                        )
                    })
                }
                <Form>
                    <Form.Group controlId='customerName'>
                        <Form.Label>Imie i nazwisko</Form.Label>
                        <Form.Control disabled type="text" value={order.name} />
                    </Form.Group>
                    <Form.Group controlId='customerAddresLine1'>
                        <Form.Label>Adres linia 1</Form.Label>
                        <Form.Control disabled type="text" value={order.addressLine1} />
                    </Form.Group>
                    <Form.Group controlId='customerAddresLine2'>
                        <Form.Label>Adres linia 2</Form.Label>
                        <Form.Control disabled type="text" value={order.addressLine2} />
                    </Form.Group>
                </Form>
            </div>
        </>
    );
}

export default withRouter(Order);
